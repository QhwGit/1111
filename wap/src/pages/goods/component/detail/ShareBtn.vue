<template>
  <div class="share-box">
    ...
    <HeadBtn
      ...
      icon="v-icon-cart1"
    />
    <HeadBtn type="share" event :style="shareBtnStyle" v-if="!showLoading" @click="onClick" />
    <div class="loading-box" :style="shareBtnStyle" v-else>
      <van-loading color="white" size="20px" />
    </div>
    <div class="image-wrapper" ref="imageWrapper" v-if="!imgUrl&&showImgHtml&&shareUrl">
      <div class="img-box">
        <div class="banner">
        ...
        <van-cell class="info-box">
          ...
          <div class="price">
            {{goodsInfo.goodsPrice | yuan}}
            <small class="market">{{ goodsInfo.marketPrice | yuan }}</small>
          </div>
        </van-cell>
        <div class="qr-box">
          <Qrcode class="img" :options="{ size: 80 }" :value="shareUrl" tag="img" id="qrImg" />
        </div>
      </div>
    </div>
    <van-popup v-model="popupShow" class="share-popup">
      ...
      <div class="foot">
        <small class="tip">
          <span>{{$store.state.isWeixin?'长按图片可保存图片或分享':'长按图片可保存图片'}}</span>
          <span>{{$store.state.isWeixin?'如需链接分享，请点击右上角进行分享':'如需链接分享，请点击下方进行分享'}}</span>
        </small>
      </div>
    </van-popup>
  </div>
    ...
    getGoodsImgBase64(goods_id) {
      return new Promise((resolve, reject) => {
        GET_GOODSIMGBASE64(goods_id)
          .then(({ data }) => {
            resolve(data[0] || this.goodsInfo.picture);
          })
          .catch(() => {
            resolve(this.goodsInfo.picture);
          });
      });
    },
    getImgUrl() {
      ...
      $this.$store
        ...
        .catch(() => {
          $this.getGoodsImgBase64($this.goodsInfo.id).then(img => {
            $this.goodsImg = img;
