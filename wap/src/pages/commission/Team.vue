...
export default sfc({
  ...
  computed: {
    ...
    tabs() {
      ...
      // }
      console.log(this.$store.state.member.commissionSetText.level_data);
      let arr = this.$store.state.member.commissionSetText.level_data;
      return arr;
    }
  },
  mounted() {
    ...
    GET_COMMISSIONSETTEXT().then(res => {});
  ...
  methods: {
    onTab(index) {
      const $this = this;
      const type = $this.tabs[index].id;
      $this.params.level_id = type;
      $this.loadList("init");
    },
    user_num(num) {
      let obj = {
