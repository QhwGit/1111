  ...
  <div>
    <van-cell is-link @click="show=true">
      ...
      <div class="value">
        ...
        <div>尾款时间：{{info.pay_start_time | formatDate}} ~ {{info.pay_end_time | formatDate}}</div>
        <div>发货时间：{{info.pay_end_time | formatDate}}</div>
      </div>
    </van-cell>
    <div>
  ...
  computed: {
    list() {
      ...
      let list = [
        {
          title: "定金",
          value: `定金支付：${formatDate(info.start_time)}至${formatDate(
            info.end_time
        ...
        {
          title: "尾款",
          value: `尾款支付：${formatDate(info.pay_start_time)}至${formatDate(
            info.pay_end_time
          )}支付尾款，超时订单关闭，且定金不予退还。`
        },
        {
          title: "发货",
          value: `发货时间：${formatDate(info.send_goods_time)}开始发货。`
        }
      ];
      return list;
    }
